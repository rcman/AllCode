	...
	if (hc == 1 && lc == 27)   // ESC Key & Exit
		...
		hc=0;lc=0;
			...
			if (hc == 81 && lc == 0 && rw < 9)        // Page Down
			{
				...
				if (chkdrw ==1 )        // you drew but didn't move forward
				 {
					animobjects[0].row+=1;
...
void filewrite(void)
{
  ...
  // printf("Open Output File \n");
	   ...
	   == NULL)
   {
	...
	// animobjects[rw].fshp[sp].shp[i*16+j];

	gotoxy (25,25);
	printf("Rows %d %d",animobjects[0].row,rw);


	for (numrow=0;numrow<=animobjects[0].row+1;numrow++)
	{
		for (numobj=0;numobj<animobjects[numrow].max;numobj++)
		{
			for (i=0;i<16;i++)
			{
				for (s=0;s<16;s++)
				{
				fputc(animobjects[numrow].fshp[numobj]->shp[i*16+s], out);
				}
			}
		}
	}
