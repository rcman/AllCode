...
int main(int argc, char* argv[]) {
  ...
  while (running) {
    ...
    if (keystate[SDL_SCANCODE_SPACE]) {
        ...
        if (bulletIndex != -1) {
          playerBullets[bulletIndex] = {player.x, player.y, 8, 8};
    ...
    for (Enemy &enemy : enemies) {
      if (enemy.active && rand() % 100 == 0) {
        ...
        if (bulletIndex != -1) {
          enemyBullets[bulletIndex] = {enemy.rect.x, enemy.rect.y, 8, 8};
    ...
    for (int i = 0; i < NUM_BULLETS; i++) {
      if (playerBullets[i].w) {
        SDL_RenderCopy(rende  SDL_RenderCopy(renderer, enemyBulletTexture, NULL, &enemyBullets[i]);
      }
    }
